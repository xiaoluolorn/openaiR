...
EmbeddingsClient <- R6::R6Class(
  ...
  public = list(
    ...
    #   \itemize{
    #     \item A single character string: "Hello world"
    ...
    #   \itemize{
    #     \item "text-embedding-3-small" — 1536 dimensions, faster, cheaper
    #     \item "text-embedding-3-large" — 3072 dimensions, highest quality
    #     \item "text-embedding-ada-002" — 1536 dimensions, legacy model
    #   }
    #   Default: "text-embedding-ada-002".
    #
    # @param encoding_format Character. Format of the returned embedding vectors:
    #   \itemize{
    #     \item "float" (default) — Returns a list of floating-point numbers
    #     \item "base64" — Returns base64-encoded compact binary string
    #   }
    #   Default: NULL (API default "float").
    #
    # @param dimensions Integer. Number of dimensions for the output embedding.
    #   Only supported by text-embedding-3-* models. Reduces vector size
    #   while preserving most semantic information.
    #   E.g. set dimensions=256 to get 256-dim vectors instead of 1536.
    #   Default: NULL (full dimensions).
    #
    ...
    #     \item{$data}{List of embedding objects (one per input):
    #       \describe{
    #         \item{$data[[i]]$embedding}{Numeric vector of floats — the embedding.
    #           Use unlist() to convert to an R numeric vector.}
    #         \item{$data[[i]]$index}{Integer. Position of this item in the input list.}
    #         \item{$data[[i]]$object}{Always "embedding".}
    #       }
    #     }
    #     \item{$model}{The model used.}
    #     \item{$usage$prompt_tokens}{Tokens consumed.}
    #     \item{$usage$total_tokens}{Total tokens.}
    #   }
    #
    # @examples
    # \dontrun{
...
#'
#' @param input **Required.** A character string or list of strings to embed.
#' @param model Character. Embedding model. Default: `"text-embedding-ada-002"`.
#' @param ... Additional parameters passed to `EmbeddingsClient$create()`,
#'   such as `dimensions` or `encoding_format`.
#'
#' @return A named list with `$data[[i]]$embedding` containing the
